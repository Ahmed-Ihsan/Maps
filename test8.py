...
# Step 2: Find two alternative routes
routes = [primary_route]
excluded_edges = []

for i in range(2):  # Find 2 additional routes
    # Get the edges of the last computed route
    route_edges = [(primary_route[j], primary_route[j + 1], 0) for j in range(len(primary_route) - 1)]
    ...
    if alternative_route is not None and alternative_route not in routes:
        ...
        print(f"Could not find alternative route {i + 1}.")
    
    # Use the new alternative route as the "primary" for the next iteration
    primary_route = alternative_route if alternative_route else primary_route

# Ensure we have exactly three routes
if len(routes) < 3:
    print("Not enough distinct routes found. Reusing the primary route.")
    while len(routes) < 3:
        routes.append(routes[0])

# Plot the graph with all three routes highlighted
fig, ax = ox.plot_graph_routes(
    G,
    routes,
    route_colors=["blue", "green", "red"],
    route_linewidths=[4, 4, 4],
    node_size=0,
    bgcolor="white",
    show=False,
    close=False
...
ax.legend(
    handles=[
        plt.Line2D([0], [0], color="blue", lw=4, label="Primary Route"),
        plt.Line2D([0], [0], color="green", lw=4, label="Alternative Route 1"),
        plt.Line2D([0], [0], color="red", lw=4, label="Alternative Route 2"),
