...
colors = ["#0074D9", "#2ECC40", "#FF4136", "#FF851B", "#B10DC9", "#F012BE", "#3D9970", "#85144B", "#001f3f", "#FFDC00"][:len(routes)]
...
ax.scatter(
    ...
    c="#000000", s=200, label="Start Point", zorder=3, edgecolor="white", linewidth=1
)
ax.scatter(
    end_location[1], end_location[0],
    c="#FFFFFF", s=200, label="End Point", zorder=3, edgecolor="black", linewidth=1
)

# Add text labels for the start and end points
ax.text(start_location[1], start_location[0], "Start", fontsize=12, color="black", ha="right", va="bottom", zorder=4)
ax.text(end_location[1], end_location[0], "End", fontsize=12, color="black", ha="left", va="top", zorder=4)
...
legend_handles.extend([
    plt.Line2D([], [], marker='o', color='w', markerfacecolor='black', markersize=10, label='Start Point', markeredgewidth=1, markeredgecolor='white'),
    plt.Line2D([], [], marker='o', color='w', markerfacecolor='white', markersize=10, label='End Point', markeredgewidth=1, markeredgecolor='black')
])

ax.legend(handles=legend_handles, loc="upper right", fontsize=10, frameon=True, framealpha=0.8, edgecolor="black")

# Add title and subtitle
ax.set_title("Multiple Routes Between Start and End Points", fontsize=16, pad=20, fontweight="bold")
ax.text(0.5, -0.05, "Generated using A* Algorithm with Edge Exclusion", transform=ax.transAxes, fontsize=10, ha="center", va="top", color="gray")
