...
# Calculate shortest paths using three algorithms
dijkstra_route = nx.dijkstra_path(G, origin_node, destination_node, weight="length")
astar_route = nx.astar_path(G, origin_node, destination_node, heuristic=great_circle_heuristic, weight="length")
bellman_ford_route = nx.bellman_ford_path(G, origin_node, destination_node, weight="length")

# Plot the graph with all three routes highlighted
fig, ax = ox.plot_graph_routes(
    G,
    [dijkstra_route, astar_route, bellman_ford_route],
...
ax.legend(
    handles=[
        plt.Line2D([0], [0], color="blue", lw=4, label="Dijkstra"),
        plt.Line2D([0], [0], color="green", lw=4, label="A*"),
        plt.Line2D([0], [0], color="red", lw=4, label="Bellman-Ford"),
